...
class Block {
 public:
  ...
  Block(int round_no, int creator_node, list<Block>::iterator pointer)
      : round(round_no), prev(pointer), creator(creator_node), strategy(-2) {
    reward = double(round_no - pointer->round) + pointer->leftover leftover =
                 (pointer->leftover + (round_no - pointer->round)) - reward;
    height = pointer->height + 1;
    max_height = max(max_height, height);
    ID = nextID++;
  }
