...
int main(int argc, char *argv[]) {
  ...
  double p = 0.01;
  auto last_round_begin = mined.begin(), last_round_end = mined.end();
  for (int i = 0; i < r; i++) {
    cerr << "Round " << i << " mined.size " << mined.size() << endl;
    for (int j = 0; j < n; j++) {
      if (R[i][j] < p) {
        for (auto it = last_round_begin; it != last_round_end; it++) {
          for (int k = 0; k < 4; k++) {
    ...
    last_round_begin = last_round_end;
