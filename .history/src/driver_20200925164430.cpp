  ...
  for (int i = 0; i < r; i++) {
    ...
    for (int j = 0; j < n; j++) {
      if (R[i][j] < p) {
        ...
        cout << " Longest Chain Length:" << longest_chain_length << endl;
        for (auto it = mined.begin(); it != mined.end(); ++it) {
          if (it->height == longest_chain_length && it->creator == -1 ||
              it->round <= i - D[it->creator][j]) {
            mined.push_back(
                Block(i, (it->creator == -1) ? i : (i - D[j][j] + 1), j, it));
          }
        }
      }
    }
