...
int main(int argc, char *argv[]) {
  ...
  for (int i = 0; i < r; i++) {
    cerr << i << endl;
    int this_round = last_round;
    for (int j = 0; j < n; j++) {
      if (R[i][j] < p) {
        cerr << i << ' ' << j << " on " << last_round << endl;
        for (auto it = mined.begin(); it != mined.end(); ++it) {
          // cerr <<
          if (it->round == last_round) mined.push_back(Block(i, j, 1, it));
          // mined.push_back(Block(i, j, it));
          this_round = i;
        }
      }
    }
  ...
  for (auto it = mined.begin(); it != mined.end(); it++) {
    if (it->height == Block::max_height) {
      ...
      //     // for (auto it2 = proposed_chain.begin(); it2 !=
      //     proposed_chain.end();
      //     //      it2++) {
      //     //   cerr << it2->reward << ',';
      //     // }
